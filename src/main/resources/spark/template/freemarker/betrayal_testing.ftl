...
<div class="game">
	...
	<div class="main_game">
		<div class="player_1">
			...
			<div class="stat">
				<center>K N O W L E D G E</center>
			</div>
...
<head>
  ...
  <style>
  #first, #second, #basement {position: relative; top:-425px; left:-275px;}
  #second, #basement {display: none;}
  #box {width:800px; height:512px; border: solid black 1px; overflow: hidden; border-width: 1px 1px 1px 0; position: relative; z-index: 0;}
  #mapf, #maps, #mapb {overflow: hidden; border:solid black 1px; border-width: 0 1px 1px 0; width:170px; height:170px;}
  #map {display: flex; flex-direction:column; border: solid black 1px; border-width: 1px 0 0 1px;}
  #container {display:flex; flex-direction:row;}
  #descr {position: absolute; right: 0; bottom: 0; width:350px; height:60px; border: solid black 1px; border-width: 1px 0 0 1px; z-index: 1; background: white;}
  </style>
...
<div> Moves: <span id="moves">10</span></div>
<script type="text/javascript">
  ...
  $("#first").mousedown(function(){b1=false;}).mousemove(function(){b1=true;}).mouseup(function(event){
    if(!b1){
        const x = event.pageX - first.offsetLeft;
        const y = event.pageY - first.offsetTop;
        const xpos = x - (x % T) - T + edgex[1];
        const ypos = y - (y % T) + edgey[1];
  ...
  $("#second").mousedown(function(){b2=false;}).mousemove(function(){b2=true;}).mouseup(function(event){
    if(!b2){
        const x = event.pageX - second.offsetLeft;
        const y = event.pageY - second.offsetTop;
        const xpos = x - (x % T) - T + edgex[2];
  ...
  $("#basement").mousedown(function(){b3=false;}).mousemove(function(){b3=true;}).mouseup(function(event){
    if(!b3){
        const x = event.pageX - basement.offsetLeft;
        const y = event.pageY - basement.offsetTop;
        const xpos = x - (x % T) - T + edgex[0];
